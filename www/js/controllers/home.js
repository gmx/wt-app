App.controller('home', function (page) {
  $(page).on('appLayout', function() {
    ...
    if (localStorage.getItem('api_token')) {
      ...
      $(page).find('#business-listing-page-link').show();
      ...
      $(page)
        .find('#cv-upload-page-link, #job-upload-page-link, .user-log-in')
        .data('target', 'login')
        .show();
      $(page).find('#cv-upload-page-link').css({ "background-color": "#21629C" });
      $(page).find('#job-search-page-link').css({ "background-color": "#5B2C6F" }).show();
